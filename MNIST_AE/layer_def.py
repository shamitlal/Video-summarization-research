...
def deconv_layer(inputs, kernel_size,output_shape,idx, stride=2,padding="SAME",batch_norm=True):
    with tf.variable_scope('{0}_deconv'.format(idx)) as scope:
      ...
      if batch_norm==True:
        ...
        conv2d_transpose = conv_rect_batch_norm
      conv_rect_batch_norm = tf.nn.leaky_relu(conv2d_transpose,alpha=0.2)
