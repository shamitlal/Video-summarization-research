...
def down_sampling_decoders(inputs,hidden_1,hidden_2,scope):
    ...
    #DECODER BLOCK 1
    conv1_1, conv_w1_1, conv_b1_1 = ld.conv_layer(inputs, 3, 2, 256, "segmentation_decode_1_1",padding="SAME")
    ld.variable_summaries(conv1_1, scope + "_conv1_1")

    # conv lstm cell 1
    shape = tf.shape(conv1_1)
    with tf.variable_scope("segmentation_conv_lstm_1"):
    ...
    #7X7X256
    #DECODER BLOCK 2
    conv2_1, conv_w2_1, conv_b2_1 = ld.conv_layer(y_1, 3, 2, 512,"segmentation_decode_2_1",padding="SAME")
    ld.variable_summaries(conv2_1, scope + "_conv2_1")


    # conv lstm cell 2
    shape = tf.shape(conv2_1)
    with tf.variable_scope('segmentation_conv_lstm_2'):
    ...
    conv5_1, conv_w5_1, conv_b5_1 = ld.conv_layer(y_2, 3, 2, 1024,"segmentation_decode_5_1",padding="SAME")
    ld.variable_summaries(conv5_1, scope + "_conv5_1")

    conv6_1, conv_w6_1, conv_b6_1 = ld.conv_layer(conv5_1, 3, 2, 2048,"segmentation_decode_6_1",padding="SAME")
    ...
    conv3_1, conv_w3_1, conv_b3_1 = ld.conv_layer(y_2, 1, 1, 512,"segmentation_decode_3_1",padding="SAME")
    # 1 X 1 X 512
    conv4_1, conv_w4_1, conv_b4_1 = ld.conv_layer(conv3_1, 1, 1, 1,"segmentation_decode_4_1",padding="SAME",linear=True)
    # 1 X 1 X 1
...
def up_sampling_decoder(inputs,skip_connection_1,scope):
    #14 X 14 X 128 
...
def network(inputs, hidden_1,hidden_2, summarization_hidden_1,summarization_hidden_2,segmentation_hidden_1,segmentation_hidden_2):
    ...
    with tf.variable_scope("segmentation"):
      segmentation_output,segmentation_hidden_feature_map_1,segmentation_hidden_feature_map_2 = down_sampling_decoders(y_2,segmentation_hidden_1,segmentation_hidden_2,"segmentation")
    

    with tf.variable_scope("prednet"):
      prednet_output,prednet_wts_list,prednet_feature_map_list = up_sampling_decoder(y_2,y_1,"prednet")
    ...
    return prednet_output,hidden_feature_map_1, hidden_feature_map_2,wts_list,feature_map_list,prednet_wts_list,prednet_feature_map_list, summarization_output,summarization_hidden_feature_map_1,summarization_hidden_feature_map_2,segmentation_output,segmentation_hidden_feature_map_1,segmentation_hidden_feature_map_2
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    segmentation_labels  = tf.placeholder(tf.float32, [None])
    ...
    stddev = 0.2
    hidden_placeholder_1 = tf.get_variable("hidden_placeholder_1",[BATCH_SIZE,56,56,64],tf.float32,initializer=tf.truncated_normal_initializer(stddev=stddev))
    hidden_placeholder_2 = tf.get_variable("hidden_placeholder_2",[BATCH_SIZE,14,14,256],tf.float32,initializer=tf.truncated_normal_initializer(stddev=stddev))
    summarization_hidden_placeholder_1 = tf.get_variable("summarization_hidden_placeholder_1",[BATCH_SIZE,7,7,512],tf.float32,initializer=tf.truncated_normal_initializer(stddev=stddev))
    summarization_hidden_placeholder_2 = tf.get_variable("summarization_hidden_placeholder_2",[BATCH_SIZE,4,4,1024],tf.float32,initializer=tf.truncated_normal_initializer(stddev=stddev))
    segmentation_hidden_placeholder_1 = tf.get_variable("segmentation_hidden_placeholder_1",[BATCH_SIZE,7,7,512],tf.float32,initializer=tf.truncated_normal_initializer(stddev=stddev))
    segmentation_hidden_placeholder_2 = tf.get_variable("segmentation_hidden_placeholder_2",[BATCH_SIZE,4,4,1024],tf.float32,initializer=tf.truncated_normal_initializer(stddev=stddev))
    ...
    with tf.variable_scope("prednet") as scope_:
        device_count = 0
        with tf.device("/gpu:" + str(device_count)):
            x_1, hidden_feature_map_1,hidden_feature_map_2,_,_,_,_,summarization_output,summarization_hidden_feature_map_1,summarization_hidden_feature_map_2,segmentation_output,segmentation_hidden_feature_map_1,segmentation_hidden_feature_map_2 = network_template(x_dropout[:,0,:,:,:], 
              ...
              segmentation_hidden_1=segmentation_hidden_placeholder_1,
              segmentation_hidden_2=segmentation_hidden_placeholder_2)
            x_unwrap.append(x_1)
            summarization_x_unwrap.append(summarization_output)
            segmentation_x_unwrap.append(segmentation_output)

        scope_.reuse_variables()
        gpu_devices = [i for i in range(0,8)]
        for i in xrange(1,SEQ_LENGTH):
            with tf.device("/gpu:" + str(device_count)):
              x_1, hidden_feature_map_1,hidden_feature_map_2, wts_list, feature_map_list, prednet_wts_list, prednet_feature_map_list,summarization_output,summarization_hidden_feature_map_1,summarization_hidden_feature_map_2,segmentation_output,segmentation_hidden_feature_map_1,segmentation_hidden_feature_map_2 = network_template(x_dropout[:,i,:,:,:], 
                ...
                segmentation_hidden_1=segmentation_hidden_feature_map_1,
                segmentation_hidden_2=segmentation_hidden_feature_map_2)
              x_unwrap.append(x_1)
              summarization_x_unwrap.append(summarization_output)
              segmentation_x_unwrap.append(segmentation_output)
            device_count+=1
    ...
    segmentation_output = tf.reshape(segmentation_x_unwrap,[-1])
    #sigmoid_summarization_output = tf.nn.sigmoid(summarization_output)
    sigmoid_summarization_output = summarization_output
    ...
    #transformed_labels = tf.floordiv(labels,16)
    x_unwrap = tf.reshape(x_unwrap,[-1,SEQ_LENGTH, IMAGE_SHAPE, IMAGE_SHAPE])
    #transformed_labels = tf.reshape(labels,[-1,SEQ_LENGTH,IMAGE_SHAPE, IMAGE_SHAPE])
    #print("labels: " + str(transformed_labels))
    #print("x_unwrap: " + str(x_unwrap))
    #print("LABELS SHAPE: " + str(transformed_labels.shape))
    print("X_UNWRAP SHAPE: " + str(x_unwrap.shape))
    #print("INPUT SHAPE: " + str(x.shape))
    #print("keep_prednet_loss shape: " + str(keep_prednet_loss.shape))
    ...
    #softmax_loss = (transformed_labels-x_unwrap)**2
    #softmax loss shape : [BATCH_SIZE,SEQ_LENGTH,IMAGE_SHAPE,IMAGE_SHAPE,1]
    reshaped_loss = tf.reshape(loss,[-1,SEQ_LENGTH,IMAGE_SHAPE*IMAGE_SHAPE*1])
    reduced_loss = tf.reduce_mean(reshaped_loss,axis=2)
    #reduced softmax loss shape: [BATCH_SIZE,SEQ_LENGTH]
    print " LOSS SHAPE: "  + str(loss.shape)
    #print " RESHAPED LOSS SHAPE: "  + str(reshaped_softmax_loss.shape)
    #print " REDUCED LOSS SHAPE: "  + str(reduced_softmax_loss.shape)
    final_loss = reduced_loss * tf.cast(keep_prednet_loss, tf.float32)
    #print " FINAL LOSS SHAPE: "  + str(final_softmax_loss.shape)
    ...
    summarization_loss = tf.square(summarization_output- summarization_labels)
    summarization_loss = tf.reduce_mean(summarization_loss)

    #loss = summarization_loss

    segmentation_loss = tf.square(segmentation_output-segmentation_labels)
    #segmentation_loss = tf.nn.sigmoid_cross_entropy_with_logits(logits=segmentation_output,labels=segmentation_labels)
        ...
        while start_count+BATCH_SIZE<=len(videos):
            ...
            while frame_start_count+SEQ_LENGTH+1<=mini_len:
                ...
                for video_index_num in range(0, video_index):
                    ...
                    scene_detection_array = [1 if video_frame_boundary_map[video_index_video_mapping[video_index_num]][frame_number.split('/')[-1]]==1 else 0 for frame_number in selected_frames]
                    scene_detection_array[0] = 1
                    ...
                    scene_detection_labels.append(scene_detection_array)
                ...
                scene_detection_labels = np.asarray(scene_detection_labels).reshape(-1)
                #print("input_data shape: " + str(input_data.shape))
                #print("prednet_output_data shape: " + str(prednet_output_data.shape))
                #print("keep_prednet_array_batch shape: " + str(keep_prednet_array_batch.shape))

           

                t = time.time()
                #frame_importance_numpy = np.asarray(frame_importance[start_count:start_count+10]).reshape(-1)
                _, loss_r, summary, hidden_last_batch_1, hidden_last_batch_2,summarization_hidden_last_batch_1, summarization_hidden_last_batch_2,summarization_output_,segmentation_hidden_last_batch_1, segmentation_hidden_last_batch_2,segmentation_output_ = sess.run([train_op, loss, tf_tensorboard,hidden_feature_map_1,hidden_feature_map_2,summarization_hidden_feature_map_1,summarization_hidden_feature_map_2,sigmoid_summarization_output,segmentation_hidden_feature_map_1,segmentation_hidden_feature_map_2,segmentation_output],
                feed_dict={x:input_data, labels:prednet_output_data,keep_prednet_loss:keep_prednet_array_batch, summarization_labels:dat_label,segmentation_labels:scene_detection_labels})
                ...
                segmentation_hidden_input_1  = segmentation_hidden_last_batch_1
                segmentation_hidden_input_2  = segmentation_hidden_last_batch_2
                ...
                matched_detection = 0
                for one,two in zip(np.asarray(segmentation_output_),np.asarray(scene_detection_labels)):
                    if one==two and two==1:
                        matched_detection += 1
                total_ones = np.sum(scene_detection_labels)
                

                print "LOSS: " + str(loss_r)
                #print("Summarization output  :  " + str(np.asarray(summarization_output_).astype(int)))
                #print("True output  :  " + str(dat_label))
                #print("Segmentation output  :  " + str(np.asarray(segmentation_output_).astype(int)))
                #print("True output  :  " + str(scene_detection_labels))
                #print("Segmentation accuracy : " +str(matched_detection) + '/' + str(total_ones) )
