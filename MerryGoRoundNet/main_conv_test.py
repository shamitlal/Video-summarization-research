...
def get_images(frames, mean_image,use_mean_image,convert_to_grey_scale, shape=IMAGE_SHAPE):
  ...
  images = [scipy.misc.imresize(imread(element),(image_rows,image_columns,image_channels)) for element in frames]
    ...
    while(epoch<MAX_EPOCHS):
        ...
        while start_count+BATCH_SIZE<=len(videos):
            ...
            while frame_start_count+SEQ_LENGTH<=mini_len:
                ...
                summarization_output_, next_frame_output,hidden_last_batch_1,hidden_last_batch_2,summarization_hidden_last_batch_1,summarization_hidden_last_batch_2 = sess.run([summarization_output,x_unwrap,hidden_feature_map_1,hidden_feature_map_2,summarization_hidden_feature_map_1,summarization_hidden_feature_map_2],
                ...
                summ_output.append(summarization_output_)
                nf_output.append(next_frame_output)

            output_images = np.array(nf_output).reshape(-1,IMAGE_SHAPE,IMAGE_SHAPE)
            summ_output = np.array(summ_output).reshape(-1)
            print("summ_output :" + str(summ_output))
            if not os.path.isdir('PREDNET_OUTPUT_FINAL'):
                os.system('mkdir ' + 'PREDNET_OUTPUT_FINAL')
            for i in range(SEQ_LENGTH):
                scipy.misc.imsave('PREDNET_OUTPUT_FINAL/outfile_' + str(i) + '.jpg', output_images[i])
