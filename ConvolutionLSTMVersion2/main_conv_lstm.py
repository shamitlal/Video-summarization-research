...
def network(inputs, hidden,hidden_feature_map, lstm=True):
  ...
  inputs = tf.reshape(inputs,[shape[0]*shape[1],shape[2],shape[3],shape[4]])
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    with tf.device("/gpu:" + str(device_count)):
        ...
        print(x_dropout)
        x_1, hidden_feature_map = network_template(x_dropout[:,:,:,:,:], hidden = hidden_placeholder,hidden_feature_map = hidden_feature_map)
        x_unwrap.append = x_1
