...
def conv_layer3D(inputs, kernel_size, stride, num_features, idx,padding='SAME' linear = False):
  with tf.variable_scope('{0}_conv'.format(idx)) as scope:
...
_variable_with_weight_decay
    conv = tf.nn.conv3d(inputs, weights, strides=[1,stride,stride, stride, 1], padding=padding)
    conv_biased = tf.nn.bias_add(conv, biases)
    if linear:
      return conv_biased
    #batchnorm should be before elu
