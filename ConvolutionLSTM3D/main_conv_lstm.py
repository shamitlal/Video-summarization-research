...
def network(inputs, hidden,hidden_feature_map, lstm=True):
  ...
  #conv3d parameters: input , [3,3,3,3] ,[1,1,1], padding="SAME"
  conv1_1 = ld.conv_layer3D(inputs, 3, 1, 16, idx="encode_1_1",padding="SAME")
  # conv3D => 5,224,224,16
  
  #conv3d parameters: input , [3,3,3] ,[1,1,1], padding="valid"
  conv1_2 = tf.nn.max_pool3d(ld.conv_layer3D(conv1_1, 3, 1, 16, "encode_1_2",padding="SAME"), [1,3,3,3,1],strides=[1,2,2,2,1], padding="SAME")
  # conv3D => 5,112,112,16
  # pool   => 3,112,112,16


  #conv3d parameters: input , [3,3,3] ,[1,1,1], padding="SAME"
  conv2_1 = ld.conv_layer3D(conv1_2, 3, 1, 32, "encode_2_1",padding="SAME")
  # conv3D => 3,112,112,32

  #conv3d parameters: input , [3,3,3] ,[1,1,1], padding="valid"
  conv2_2 = tf.nn.max_pool3d(ld.conv_layer3D(conv2_1, 3, 1, 32, "encode_2_2",padding="SAME"), [1,3,3,3,1],strides=[1,2,2,2,1], padding="VALID")
  # conv3D => 3,112,112,32
  # 1,64,64,32
  ...
  conv2_2 = tf.reshape(conv2_2,[shape[0],shape[2],shape[3],shape[4]])
  print("shape before lstm: " + str(shape))
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    with tf.device("/gpu:" + str(device_count)):
        x_1, hidden_feature_map = network_template(x_dropout[:,0:IMAGE_PER_TIMESTEP,:,:,:], hidden = hidden_placeholder,hidden_feature_map = hidden_feature_map)
        x_unwrap.append(x_1)

    gpu_devices = [i for i in range(0,8)]
    for i in xrange(1 + IMAGE_PER_TIMESTEP//2,SEQ_LENGTH-IMAGE_PER_TIMESTEP//2):
        with tf.device("/gpu:" + str(device_count)):
          x_1, hidden_feature_map = network_template(x_dropout[:,i-IMAGE_PER_TIMESTEP//2:i+1+IMAGE_PER_TIMESTEP//2,:,:,:], hidden = hidden_feature_map,hidden_feature_map = hidden_feature_map)
          x_unwrap.append(x_1)
        device_count+=1
        device_count%=1
    ...
    while(epoch<MAX_EPOCHS):
      ...
      while start_count+BATCH_SIZE<len(videos):
        ...
        prev_frames = []
        mean_image = "black_image.jpg"
        for prev_frame in range(IMAGES_PER_TIMESTEP//2):
          prev_frames.append(mean_image)
        ...
        for video in videos_in_iteration:
          ...
          frame[video_index] = prev_frames + frame[video_index] + prev_frames
        ...
        while frame_start_count+SEQ_LENGTH<=mini_len:
          ...
          for video_index_num in range(0, video_index):
            selected_frames = frame[video_index_num][frame_start_count: frame_start_count+SEQ_LENGTH+IMAGES_PER_TIMESTEP-1]
          ...
          _, loss_r, accuracy_r, summary, output, hidden_last_batch = sess.run([train_op, loss, accuracy, tf_tensorboard, output_integer,hidden_feature_map],feed_dict={x:dat, labels:dat_label,hidden_placeholder:hidden_input})
          elapsed = time.time() - t
